...
**Upgrade visual + analítico (15/06/2026):** a aba ganhou — (a) **heatmap de cobertura** (bloco 0): top 26 SKUs × loja, célula verde/âmbar/vermelho por status (herói curva A / vende fraco / parado-ausente); (b) **scatter margem × giro** (`scatterSVG`): cada SKU um ponto (x = giro em log, y = margem, raio ∝ faturamento, cor = quadrante), com linhas de referência (margem do canal, giro mediano) e `<title>` por ponto — os cards de quadrante seguem abaixo; (c) **barra de amplitude de preço** (`rangeBarSVG`) no bloco de preço; (d) **estimativa de lacuna normalizada por tráfego** — penetração (un/cupom) da loja-herói × cupons desta loja × preço, em vez de escalar por GMV (usa `l.boletos`); (e) **roll-up das lacunas por marca** (barras). Helpers novos: `scatterSVG`, `rangeBarSVG`. **Validação:** Chromium — heatmap 156 células, scatter 1.742 pontos, 6 barras de marca, 12 de preço, bugcheck completo (5 abas + rename + XSS) zero erros de console.
